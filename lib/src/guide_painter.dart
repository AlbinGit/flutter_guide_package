...
class MyCustomPainter extends CustomPainter {
  ...
  void paint(ui.Canvas canvas, ui.Size size) {
    ...
    if (tipsAlign == TipsAlign.topLeft) {
      ...
      double centerPointX =
          starPoint.x + pading + triangleLengh - tipsWidth / 2.0;
      ...
      centerPoint = TipsPoint(
          centerPointX, starPoint.y - triangleLengh - tipsHeight / 2.0);
      ...
      tipsHeight = getRectHeight(canvas, pading);
      centerPoint = TipsPoint(centerPointX,
          starPoint.y - triangleLengh - pading + tipsHeight / 2.0);
    ...
    ///绘制提示文本
    Offset tipsOrigin = Offset(centerPoint.x - tipsWidth / 2.0 + pading,
        centerPoint.y - tipsHeight / 2.0 + pading);
    Size tipsSize;
    drawTextFunction(tipsWidth - pading * 2, tipsOrigin, canvas, textTip,
        textColor: tipsTextColor,
        textSize: tipsTextSize, drawedCallback: (Size size) {
      tipsSize = size;
    });

    ///绘制下一步文本
    Offset nextOrigin =
        Offset(tipsOrigin.dx, tipsOrigin.dy + tipsSize.height + pading);
    drawTextFunction(tipsWidth - pading * 2, nextOrigin, canvas, nextTextTip,
        textColor: nextTextColor,
        textSize: nextTextSize,
        textAlign: TextAlign.right, drawedCallback: (Size size) {
      ///点击区域位置记录
      if (clickLiser != null) {
        clickLiser(Rect.fromLTWH(
            nextOrigin.dx, nextOrigin.dy, size.width, size.height));
      }
    });
  ...
  void drawTriangle(Canvas canvas, TipsPoint starPoint, TipsAlign curverAlign,
      double triangleLengh) {
    var paint = Paint();
    ...
    TipsPoint b1;
    TipsPoint b2;
    if (curverAlign == TipsAlign.topLeft ||
        curverAlign == TipsAlign.topCenter ||
        curverAlign == TipsAlign.topRight) {
      ...
      b1 = TipsPoint(starPoint.x + triangleLengh, starPoint.y - triangleLengh);
      b2 = TipsPoint(starPoint.x + triangleLengh, starPoint.y + triangleLengh);
    } else if (curverAlign == TipsAlign.bottomLeft ||
        curverAlign == TipsAlign.bottomCenter ||
        curverAlign == TipsAlign.bottomRight) {
      a1 = TipsPoint(starPoint.x - 1, starPoint.y + 1);
      a2 = TipsPoint(starPoint.x + 1, starPoint.y + 1);
      b1 = TipsPoint(starPoint.x + triangleLengh, starPoint.y + triangleLengh);
      b2 = TipsPoint(starPoint.x - triangleLengh, starPoint.y + triangleLengh);
  ...
  void drawRectangle(
      Canvas canvas, TipsPoint centerPoint, double width, double height) {
    var paint = Paint();
    paint.color = tipsBackgroundColor;
    Rect rect = Rect.fromCenter(
        center: Offset(centerPoint.x, centerPoint.y),
        width: width,
        height: height);
    //根据上面的矩形,构建一个圆角矩形
  ...
  double getRectHeight(Canvas canvas, double pading) {
    ...
    //计算提示框高度
    Size size1 = getTextSize(tipsWidth - pading * 2, canvas, textTip,
        textSize: tipsTextSize);
    tipsHeight += size1.height;
    Size size2 = getTextSize(tipsWidth - pading * 2, canvas, nextTextTip,
        textSize: nextTextSize);
  ...
  void drawTextFunction(
      ...
      Function(Size size) drawedCallback}) {
    ///创建画笔
    var textPaint = Paint();
